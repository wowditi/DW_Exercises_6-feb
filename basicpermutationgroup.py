...
def SchreierGenerators(generators,el):
	...
	for g in range(len(generators)):
		...
		for i in generators[g]:
			if i in O:
				dictionary[g].append(-U[O.index(i)])
	for ind in range(len(O)):
		...
		for P in range(len(generators)):
			...
			print(dictionary[P][ind])
			newgen = dictionary[P][ind]*generators[P]*U[ind]
...
def Stabilizer(generators,el):
	...
	"""
	timer = time.clock()
	schreiers = SchreierGenerators(generators,el)
	print(time.clock()-timer)
	reduce = Reduce(schreiers,0)
	return reduce
